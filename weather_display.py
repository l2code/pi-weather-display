...
def render_weather_to_image(data, epd_width=400, epd_height=300):
    ...
    if not data:
        ...
        draw_header(draw, epd_width, epd_height, LOCATION_NAME)
        draw_current_conditions(draw, curr, daily, epd_width, is_night, black_img)
        draw_forecast(draw, daily, epd_width, is_night, black_img)
        draw_update_time(draw, update_time, epd_width, epd_height)
...
def update_display():
    try:
        ...
        epd.init()
        epd.Clear()
        black_img, red_img = render_weather_to_image(data, epd.width, epd.height)
